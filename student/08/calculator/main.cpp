...
int main() {
    ...
    while ( true ) {
        ...
        string inputCmd = pieces.at(0);
        // IMPORTANT: case conversion using transform
        std::transform(inputCmd.begin(), inputCmd.end(), inputCmd.begin(), ::toupper);

        // TODO: Implement command execution here!
        bool hasFound = false;
        for (const Command& command : COMMANDS) {
            if (command.str == inputCmd) {
                hasFound = true;
                if (pieces.size() - 1 == command.parameter_number) {
                    ...
                    if (string_to_double(pieces.at(1), first) and string_to_double(pieces.at(2), second)) {
                        std::cout << command.action(first, second) << std::endl;
